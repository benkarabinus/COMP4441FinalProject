...
Daily weather records from the United States are compiled from a dozen separate data sets archived at the National Centers of Environmental Information (NCEI), a branch of the National Oceanic and Atmospheric Administration (NOAA). NCEI is responsible for hosting and providing access to one of the most significant stores of environmental data in existence with over 37 petabytes of oceanic, atmospheric and geophysical data (National Centers for Environmental Information, 2019).
...
#### Visualize Missing Values From Each Data Set
...
* Decomposition of time series from the city of Salt Lake City shows that the series for average monthly temperature, total monthly precipitation, and total monthly snowfall each exhibit a strong seasonal component similar to the time series data from Albuquerque and Denver. It appears that that Salt Lake was experiencing abnormal weather patterns around 2016 with major spikes in average monthly temperature, total monthly precipitation, and total monthly snowfall.
...
### Outcome of SARIMA Analysis

#### Analysis Summary

The performance of the SARIMA models generated using the auto.arima function in R was highly dependent on the data being tested. Fitted models for total monthly precipitation and total monthly snowfall produced a confidence interval that mirrored the behavior of past observations, but exact point forecasts were much less accurate and leveled of quickly as the time horizon of the predicted values increased. More in depth analysis of the behavior of the total monthly precipitation and total monthly snowfall times series should be conducted to determine a better fit for the SARIMA model or more likely to determine a more appropriate method to model the data. Fitted models for average monthly temperature performed much better with both predicted point forecasts and. Confidence intervals mirroring past observations of average monthly temperature.

#### Research Questions: Key Takeaways

* Performing a time series analysis on the data did not reveal any significant increasing or decreasing trends in average monthly temperature, total monthly precipitation, or total monthly snowfall. There did appear to be a small increase in average monthly temperature in both Albuquerque and Phoenix around 2016, but not enough of an increase to make any consequential inferences.
* The SARIMA model created using the auto.arima function in R turned out to be fairly effective at modeling average monthly temperatures observed in the four corners region of the United States between the years 1970 and 2019. Considering the clear-cut patterns in the data for average monthly temperature in each of the four cities being analyzed and the approximately normal distribution of residuals for each of the temperature models. It’s safe to say average monthly temperature was a suitable candidate for SARIMA analysis. SARIMA modeling did not work nearly as well when used to model total monthly precipitation or total monthly snowfall. Regardless of the seasonal parameters applied by the auto.arima function in R the resulting SARIMA model was unable to approximate past observations of total monthly precipitation or total monthly snowfall effectively
